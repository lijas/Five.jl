...
function constitutive_driver(m::HyperElasticMaterial, C::SymmetricTensor{2,2,T}, state::AbstractMaterialState) where T
    @assert(PLANE_STRAIN == PLANE_STATE::PLANE_STATE_2D)

    #Convert to 3d
    C₃ = SymmetricTensor{2,3,T,6}((C[1,1], zero(T), C[1,2], one(T), zero(T), C[2,2]))
    S₃, ∂S∂C₃, newstate₃ = constitutive_driver(m, C₃, state)
...
function constitutive_driver(m::AbstractMaterial, ε_2d::SymmetricTensor{2,2,T}, prev_state::AbstractMaterialState) where T

    ε = SymmetricTensor{2,3,T,6}((ε_2d[1,1], T(0.0), ε_2d[1,2], T(0.0), T(0.0), ε_2d[2,2]))

    local σ, C, state
    if PLANE_STRESS == PLANE_STATE::PLANE_STATE_2D
        #Newton variables
        NEWTON_TOL = 1e-8
        newton_counter = 0
        ...
        while _error > NEWTON_TOL; 
            newton_counter +=1
            _σⱽ, _Cⱽ, state = constitutive_driver(m, ε, prev_state)
            σⱽ = tovoigt(_σⱽ)
            Cⱽ = tovoigt(_Cⱽ)
        ...
        σ, C, state = constitutive_driver(m, ε, prev_state)
    end
